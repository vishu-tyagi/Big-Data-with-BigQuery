  ...
  create or replace view `test-project-kkj`.`staging`.`stg_yellow_tripdata`
  OPTIONS()
  as 
...
with type_casted as (
    select
        ...
        -- timestamps
        , cast(tpep_pickup_datetime as timestamp) pickup_datetime
        , cast(tpep_dropoff_datetime as timestamp) dropoff_datetime
    ...
    from `test-project-kkj`.`staging`.`yellow_tripdata`
)
