    ...
    create or replace table `test-project-kkj`.`prod`.`fact_trips`
...
, yellow_trips as (
    ...
    from `test-project-kkj`.`staging`.`stg_yellow_tripdata`
)
, trips_unioned as (
    select *
    from green_trips
...
from
    trips_unioned trips
    inner join `test-project-kkj`.`staging`.`dim_taxi_zone_lookup` pickup
    on trips.pickup_locationid = pickup.locationid
    inner join `test-project-kkj`.`staging`.`dim_taxi_zone_lookup` dropoff
